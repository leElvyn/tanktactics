...
class ArrowButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await cog.log(self.view.game, f"{ctx.author.mention} moved his tank to {current_coordinates['x']}/{current_coordinates['y']}") 
...
class UpgradeRangeView(View):
    ...
    def __init__(self, cog_object, ctx, player_data, game):
        ...
        if is_disabled:
            ...
            self.add_item(NumberSelect(ap_number=player_data["tank"]["action_points"], placeholder="Number of tiles by whitch you want to upgrade your range." if not is_disabled else "You have no action points left !", disabled=is_disabled))
    ...
    async def number_select_callback(self, range_value, interaction: discord.Interaction):
        """called by the Select. It is the end of the tranfer flow."""
        cog = self.cog
        ctx = self.ctx
        confirm_view = Confirm()
        followup = interaction.followup
        mess = await interaction.response.send_message(f"Are you sure you want to upgrade your range by {range_value} ? This will use {range_value} action points.", ephemeral=True, view=confirm_view)
        ...
        data = {"upgrade_size": int(range_value)}
        async with cog.session.get(url, json=data) as resp:
            reply = await resp.json()
            if resp.status == 200:
                ...
                await cog.log(self.game, f"{self.author} upgraded his range by {range_value}")
