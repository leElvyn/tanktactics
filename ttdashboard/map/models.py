...
class ShootEvent(BaseEvent):
    offensive_player = models.ForeignKey("Player", on_delete=models.CASCADE, verbose_name=_('Offensive Player'), related_name="offensive_player")
    defensive_player = models.ForeignKey("Player", on_delete=models.CASCADE, verbose_name=_('Defensive Player'))

    #these are the tanks at the time of the shoot
    offensive_tank = models.ForeignKey("Tank", on_delete=models.PROTECT, verbose_name=_('Tank'), related_name='shoot_offensive_tank', null=True)
    defensive_tank = models.ForeignKey("Tank", on_delete=models.PROTECT, verbose_name=_('Tank'), related_name='shoot_defensive_tank', null=True)


class TransferEvent(BaseEvent):
    offensive_player = models.ForeignKey("Player", on_delete=models.CASCADE, verbose_name=_('Offensive Player'), related_name="transfer_offensive_player")
    defensive_player = models.ForeignKey("Player", on_delete=models.CASCADE, verbose_name=_('Defensive Player'))

    #these are the tanks at the time of the shoot
    offensive_tank = models.ForeignKey("Tank", on_delete=models.PROTECT, verbose_name=_('Tank'), related_name='transfer_offensive_tank', null=True)
    defensive_tank = models.ForeignKey("Tank", on_delete=models.PROTECT, verbose_name=_('Tank'), related_name='transfer_defensive_tank', null=True)
...
class Player(models.Model):
    ...
    ad_vote = models.ForeignKey('Player', on_delete=models.SET_NULL, verbose_name=_('Daily Vote for this dead player'), null=True, blank=True) # If the player is dead, vote for the curent action day

    player_color = models.CharField(_("Player's color, in format 'rgb(x, x, x)'"), max_length=18)
    ...
    def move(self, x, y):
        ...
        if game_object.players.filter(tank__x=position_x, tank__y=position_y).count() > 1:
            # the 1 is you
        ...
        MoveEvent.objects.create(player=self, game = game_object, new_x=x, new_y=y, old_x=self.tank.x, old_y=self.tank.y)
    ...
    def shoot(self, deffensive_player):
        ...
        if self.is_dead or deffensive_player.is_dead:
            raise Exception(_('Player is dead'))
        
        if self.tank.action_points <= 0:
            raise Exception(_('Player does not have enough action points'))

        if utils.math.get_distance(self.tank, deffensive_player.tank) > self.tank.range:
            raise Exception(_('Player is not in range'))
        
        self.tank.action_points -= 1
        
        deffensive_player.tank.health_points -= 1
        reply["deffender_health"] = deffensive_player.tank.health_points
        if deffensive_player.tank.health_points <= 0:
            deffensive_player.is_dead = True
            ...
            reply["deffensive_player_dead"] = True
        
        event = ShootEvent.objects.create(game=self.game_set.first(), offensive_player=self, defensive_player=deffensive_player, offensive_tank=self.tank, defensive_tank=deffensive_player.tank)
        event.save()
    ...
    def shoot_ap(self, deffensive_player, number_of_ap_to_shoot):
        ...
        deffensive_player.tank.action_points += number_of_ap_to_shoot

        event = TransferEvent(game=self.game_set.first(), offensive_player=self, defensive_player=deffensive_player, offensive_tank=self.tank, defensive_tank=deffensive_player.tank)
        event.save()
        deffensive_player.tank.save()
        self.tank.save()
        ...
        reply = {
            ...
            "new_defender_ap": deffensive_player.tank.action_points
        }
        return reply
...
class Tank(models.Model):
    id = models.BigIntegerField(primary_key=True)
    player = models.OneToOneField(Player, on_delete=models.PROTECT, verbose_name=_('Player'), null=True)
...
class Game(models.Model):
    ...
    game_talk_channel = models.BigIntegerField(verbose_name=_('Game discussions channel ID'))
    ...
    is_action_day_1d = models.BooleanField(verbose_name=_('Is an Action Day lasting 1 day ?'))
    ad_duration = models.IntegerField(verbose_name=_('Duration of the AD in minutes'), default=datetime.timedelta(days=1).total_seconds() / 60)

    is_started = models.BooleanField(verbose_name=_('Is the game started ?'), default=False)
    ...
    def start_game(self):
        ...
        self.save()
        self.next_ad_end = self.game_start_date + datetime.timedelta(seconds=self.ad_duration)
        ...
        for player in self.players.all():
            tank = Tank(player=player, x = random.randint(0, grid_size_x), y = random.randint(0, grid_size_y))
            tank.save()
            tank.save()
...
def broadcast_event(game: Game, event_type, data):
    ...
    async_to_sync(layer.group_send)(
        f'game_{game.guild_id}',
        {
            'type': event_type,
            'data': data,
            "new_game_data": game_serialized.data
        }
    )
